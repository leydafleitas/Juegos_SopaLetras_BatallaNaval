...
class BatallaNaval:
    ...
    def verificar_disponibilidad(self, items):
        # Obtener las coordenadas del barco actual
        nuevas_coords = self.calcular_coordenadas_finales(items)
        
        # Verificar si alguna de las coordenadas está ocupada por otro barco fijado
        for coord in nuevas_coords:
            if coord in self.obtener_coordenadas_fijadas():
                return False
        
        return True

    def obtener_coordenadas_fijadas(self):
        # Retorna un conjunto de coordenadas ocupadas por los barcos fijados
        coordenadas_fijadas = set()
        for barco in self.barcosfijados:
            coordenadas_fijadas.update(self.posicionbarcos[barco])
        return coordenadas_fijadas

    def calcular_coordenadas_finales(self, items):
        # Calcula las coordenadas finales basadas en la posición y ángulo actuales
        nuevas_coords = []
        for item in items:
            x1, y1, x2, y2 = self.canvas.coords(item)
            new_x, new_y = self.MatrizACoordenadas(int((x1 + x2) // 2 - self.start_x) // self.tamanyocelda, int((y1 + y2) // 2 - self.start_y) // self.tamanyocelda)
            nuevas_coords.append((new_x, new_y))
