...
class BatallaNaval:
    ...
    def crearbarcos(self):
        ...
        for barco in self.barcos:
            # Dibujar cada parte del barco
            barco["ids"] = []
            for (x, y) in barco["coords"]:
                # Convertir las coordenadas de la celda a coordenadas de píxeles
                x1 = coord[0] * self.tamanyocelda
                y1 = coord[1] * self.tamanyocelda
                x2 = x1 + self.tamanyocelda
                y2 = y1 + self.tamanyocelda
                # Dibujar el rectángulo que representa cada parte del barco en el canvas
                barco_id = self.canvas.create_rectangle(x1, y1, x2, y2, fill="pink")
                # Hacer que las partes del barco sean interactivas (se puedan arrastrar y rotar)
                self.canvas.tag_bind(barco_id, "<ButtonPress-1>", self.inicio)
                self.canvas.tag_bind(barco_id, "<B1-Motion>", self.mover)
                self.canvas.tag_bind(barco_id, "<Button-3>", self.rotar)
                # Guardar la posición inicial del barco en el diccionario
                self.posicionbarcos[barco_id] = coord
                self.angulo_barcos[barco_id] = 0
